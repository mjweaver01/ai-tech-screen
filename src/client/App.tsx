        ...
        <div className="max-w-5xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-semibold text-slate-900">Thoughtful AI</h1>
                <p className="text-sm text-slate-600">Healthcare Automation Support</p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          ...
          <div className="h-[600px] overflow-y-auto p-6 space-y-6">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center px-4">
                ...
                <h2 className="text-2xl font-semibold text-slate-900 mb-3">Welcome to Thoughtful AI Support</h2>
                <p className="text-slate-600 mb-8 max-w-md">
                  I'm here to help you learn about our AI-powered healthcare automation agents. Ask me anything!
                </p>
                
                <div className="w-full max-w-2xl">
                  <p className="text-sm font-medium text-slate-700 mb-4">Try asking about:</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <button
                      onClick={() => handleInputChange({ target: { value: "What does EVA do?" } } as any)}
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <div className="font-medium text-slate-900 group-hover:text-indigo-700">Eligibility Verification</div>
                          <div className="text-sm text-slate-600">Learn about EVA</div>
                        </div>
                      </div>
                    </button>
                    
                    <button
                      onClick={() => handleInputChange({ target: { value: "What does CAM do?" } } as any)}
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <span className="text-2xl">📋</span>
                        <div>
                          <div className="font-medium text-slate-900 group-hover:text-emerald-700">Claims Processing</div>
                          <div className="text-sm text-slate-600">Learn about CAM</div>
                        </div>
                      </div>
                    </button>
                    
                    <button
                      onClick={() => handleInputChange({ target: { value: "How does PHIL work?" } } as any)}
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <span className="text-2xl">💳</span>
                        <div>
                          <div className="font-medium text-slate-900 group-hover:text-purple-700">Payment Posting</div>
                          <div className="text-sm text-slate-600">Learn about PHIL</div>
                        </div>
                      </div>
                    </button>
                    
                    <button
                      onClick={() => handleInputChange({ target: { value: "What are the benefits of using your agents?" } } as any)}
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <span className="text-2xl">✨</span>
                        <div>
                          <div className="font-medium text-slate-900 group-hover:text-amber-700">Benefits & ROI</div>
                          <div className="text-sm text-slate-600">Why Thoughtful AI?</div>
                        </div>
                      </div>
                    </button>
                  </div>
            ...
            {messages.map((message, index) => {
              ...
              >
                ...
                {/* Message Content */}
                <div className={`flex-1 max-w-[75%] flex flex-col ${message.role === 'user' ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`text-xs text-slate-500 mt-2 px-1 flex items-center gap-1.5`}>
                    <span className="font-medium">{message.role === 'user' ? 'You' : 'Thoughtful AI'}</span>
            ...
            {isLoading && !messages.some((m, idx) => idx === messages.length - 1 && m.role === 'assistant') && (
          ...
          <div className="border-t border-slate-200 bg-slate-50 p-4">
            ...
            <div className="mt-3 text-center text-xs text-slate-500">
              {config ? (
                <>
                  Powered by <span className="font-medium text-slate-700">{config.provider}</span>
                  {' • '}
                  Model: <span className="font-medium text-slate-700">{config.model}</span>
                </>
              ) : (

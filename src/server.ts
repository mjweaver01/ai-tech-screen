...
const server = serve({
  ...
  routes: {
    ...
    '/api/config': {
      GET() {
        return Response.json({
          provider: process.env.LLM_BASE_URL?.includes('localhost') ? 'LM Studio' : 'OpenAI',
...
console.log(`📡 Provider: ${process.env.LLM_BASE_URL?.includes('localhost') ? 'LM Studio (Local)' : 'OpenAI (Cloud)'}`);

...
export function generateSystemPrompt(match: KnowledgeBaseMatch | null): string {
  ...
  if (match) {
    ...
    `;
     ...
     console.log(`[Knowledge Base] Matched question with ${(match.similarity * 100).toFixed(1)}% similarity`);
  } else {
     ...
     console.log('[Knowledge Base] No strong match found, using general LLM knowledge');
  }
  return systemPrompt;
}